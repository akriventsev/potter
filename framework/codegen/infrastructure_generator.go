...
// generateMigrations генерирует SQL миграции в формате up/down
func (g *InfrastructureGenerator) generateMigrations(spec *ParsedSpec, _ *GeneratorConfig) error {
	var upContent strings.Builder
	var downContent strings.Builder

	upContent.WriteString("-- Migration UP: Create tables and indexes\n")
	upContent.WriteString("-- Generated by potter-gen\n\n")

	downContent.WriteString("-- Migration DOWN: Drop tables and indexes\n")
	downContent.WriteString("-- Generated by potter-gen\n\n")
	...
	for _, agg := range spec.Aggregates {
		...
		// UP: CREATE TABLE
		upContent.WriteString(fmt.Sprintf("-- Таблица %s\n", strings.ToLower(agg.Name)))
		upContent.WriteString(fmt.Sprintf("CREATE TABLE IF NOT EXISTS %s (\n", tableName))
		upContent.WriteString("\tid VARCHAR(255) PRIMARY KEY,\n")
		...
		for _, field := range agg.Fields {
			...
			upContent.WriteString(fmt.Sprintf("\t%s %s", g.converter.ToSnakeCase(field.Name), sqlType))
			if field.Name == "id" || !field.Optional {
				upContent.WriteString(" NOT NULL")
			}
			upContent.WriteString(",\n")
		}

		upContent.WriteString("\tcreated_at TIMESTAMP NOT NULL DEFAULT NOW(),\n")
		upContent.WriteString("\tupdated_at TIMESTAMP NOT NULL DEFAULT NOW()\n")
		upContent.WriteString(");\n\n")

		// UP: CREATE INDEX
		upContent.WriteString(fmt.Sprintf("-- Индексы для таблицы %s\n", tableName))
		upContent.WriteString(fmt.Sprintf("CREATE INDEX IF NOT EXISTS %s ON %s(id);\n\n",
			indexName, tableName))
	...
	// DOWN: DROP INDEXES и DROP TABLES (в обратном порядке)
	for i := len(tables) - 1; i >= 0; i-- {
		table := tables[i]
		// Сначала удаляем индексы
		for j := len(table.indexes) - 1; j >= 0; j-- {
			downContent.WriteString(fmt.Sprintf("DROP INDEX IF EXISTS %s;\n", table.indexes[j]))
		}
		// Затем удаляем таблицу
		downContent.WriteString(fmt.Sprintf("DROP TABLE IF EXISTS %s;\n\n", table.tableName))
	}

	// Записываем up файл
	upPath := "migrations/001_create_tables.up.sql"
	if err := g.writer.WriteFile(upPath, upContent.String()); err != nil {
		return fmt.Errorf("failed to write up migration: %w", err)
	}

	// Записываем down файл
	downPath := "migrations/001_create_tables.down.sql"
	if err := g.writer.WriteFile(downPath, downContent.String()); err != nil {
