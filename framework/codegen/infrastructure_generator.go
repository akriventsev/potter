...
// generateMigrations генерирует SQL миграции
func (g *InfrastructureGenerator) generateMigrations(spec *ParsedSpec, _ *GeneratorConfig) error {
	var content strings.Builder

	content.WriteString("-- Migrations generated by potter-gen\n\n")
	...
	for _, agg := range spec.Aggregates {
		...
		content.WriteString(fmt.Sprintf("CREATE INDEX IF NOT EXISTS idx_%s_id ON %s(id);\n\n",
			tableName, tableName))
	}
